...
for (i in seq_along(n_parts)) {
  ...
  for (j in 1:no_pairsub) {
    ...
    names(z_w_dash) <- paste0("wilr", 1:length(z_w_dash))
    
    # prediction
    pred_y_dotdot <- t(as.matrix(c(z_b_dotdot, z_w_dotdot)))
    pred_y_w_dash <- t(as.matrix(c(z_b_dotdot, z_w_dash)))
    
    pred_newdat <- rbind(pred_y_dotdot, pred_y_w_dash)
    
    gamma1_hat <- as.matrix(fixef(object$Model)[, "Estimate"])
    z_0 <- cbind(Intercept = 1, pred_newdat)
    # all(colnames(z_0) == rownames(gamma1_hat))
    delta_y_w_manual <- z_0 %*% gamma1_hat
    diff_delta_y_w_manual <-
      delta_y_w_manual[2] - delta_y_w_manual[1]
    ...
    diff_delta_y_w <- diff(delta_y_w[, "Estimate"])
    
    out[[j]] <-
      c(diff_delta_y_w_manual,
        diff_delta_y_w)
  }
  substutitution_gt_w[[i]] <- out
}
str(substutitution_gt_w)