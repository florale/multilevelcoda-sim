---
title: "Diagnostics of Bayesian Compositional Multilevel Models"
author: "Flora Le"
date: "`r Sys.Date()`"
output: 
  html_document:
...
source("/Users/florale/Library/CloudStorage/OneDrive-MonashUniversity/GitHub/Projects/multilevelcoda-sim/simmodel_input.R") # groundtruth, conditions and functions
...
```{r fit, eval = run, results = "hide", echo = TRUE}
fit <- brmcoda(cilr, 
...
Further inspection of the pairs plots of the MCMC draws (below) showed that 
here are some autocorrelation between the constant-intercept
and the constant coefficients for the first between $ilr$ coordinate ($b_{bilr1}$),
and the constant-intercept
and the constant coefficients for the second between $ilr$ coordinate ($b_{bilr2}$).

```{r pairs-fit, warning=FALSE}
pairs(fit)
```


We can also use `plot()` fucntion to check both density and trace lots for these parameters.
There is some areas of the samples are drawn from outside of the target distribution,
but no strong evidence for non-convergence.

```{r plot-fit, fig.height=8}
plot(fit, variable = c("b_Intercept", "b_bilr1", "b_bilr2", "sd_ID__Intercept"), regex = TRUE)
```

# Improving within-chain autocorrelation
## Increased posterior draws
As a first step, we test if increasing iterations and warmups help.

```{r fit-it, eval = run, results = "hide", echo = TRUE}
fit_it <- brmcoda(cilr, 
               bf,
               cores = 4,
               chains = 4,
...
```{r run-fit-it, include = FALSE}
if (run) {
  saveRDS(fit_it, file = "fit_it.RDS")
} else {
  fit_it <- readRDS("fit_it.RDS")
}
```
...
However, 
@betancourt2015 point out that correlation
...
we are going to test if centered parameterisation improves the correlation.
...
```{r fit-centered, eval = run, results = "hide", echo = TRUE}
...
```{r run-fit-centered, include = FALSE}
if (run) {
  saveRDS(fit_centered, file = "fit_centered.RDS")
} else {
  fit_centered <- readRDS("fit_centered.RDS")
}
```


```{r sum-fit-centered, message=FALSE}
...
```{r pair-fit-centered, warning=FALSE}
plot(bm_manual, pars = c("b_Intercept", "b_bilr1", "b_bilr2", "sd_ID__Intercept"), regex = TRUE)
```

The last method I'm going to try to improve the within-chain correlation is a
soft parameterisation that satisfying sum-to-zero constraint.
We will also do this by manually editing Stan code.

```{r mod-stancode, warning = FALSE}
m_scentered <- cmdstan_model("fit_scentered.stan")
print(m_scentered)
```
...
```{r fit-centered, eval = run, results = "hide", echo = TRUE}
fit_scentered <- m_scentered$sample(data = sdat,
                                    parallel_chains = 4,
                                    iter_sampling = 2500,
                                    iter_warmup = 500)
```
...
```{r run-fit-centered, include = FALSE}
if (run) {
  saveRDS(fit_scentered, file = "fit_scentered.RDS")
} else {
  fit_scentered <- readRDS("fit_scentered.RDS")
}
```


```{r sum-fit-centered, message=FALSE}
bm_scentered <- brm(sleepy ~ bilr1 + bilr2 + wilr1 + wilr2 + (1 | ID), data = dat, empty = TRUE)
bm_scentered$fit <- rstan::read_stan_csv(fit_scentered$output_files())
bm_scentered <- rename_pars(bm_scentered)
summary(bm_scentered)
plot(bm_scentered, pars = c("b_Intercept", "b_bilr1", "b_bilr2", "sd_ID__Intercept"), regex = TRUE)
```
This model took a lifetime so I would not recommend it.

## References
Nicenboim, B., Schad, D., & Vasishth, S. (2023). An introduction to Bayesian data analysis for cognitive science. Under contract with Chapman and Hall/CRC statistics in the social and behavioral sciences series.
